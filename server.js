...
const LOCAL_MONGO_URI = process.env.LOCAL_MONGODB_URI || 'mongodb://127.0.0.1:27017';
...
async function initializeDb() {
  ...
  if (MONGO_URI) {
    ...
    console.warn('No MONGODB_URI provided; trying local MongoDB if available.');
  }
  candidates.push({
    uri: LOCAL_MONGO_URI,
    label: 'local MongoDB'
  ...
  for (const candidate of candidates) {
    try {
      dbClient = new MongoClient(candidate.uri, {
        serverApi: {
      ...
      console.log(`Connected to ${candidate.label} database:`, MONGO_DB_NAME);
      return;
    } catch (err) {
      console.warn(`Failed to connect to ${candidate.label} at ${candidate.uri}:`, err.message || err);
    }
  }

  console.warn('Unable to connect to MongoDB Atlas or local MongoDB; falling back to local JSON persistence.');
}
